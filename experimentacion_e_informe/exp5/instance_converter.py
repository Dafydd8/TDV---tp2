...
for j in range(2):
    for tipo in ['baja', 'alta']:
        for i in range(10):
            ...
            instance['cost_per_unit'] = {'Tigre' : 1.0, 'Retiro' : float(j+1)}


            filename = os.path.join(os.path.dirname(__file__), f'media_{tipo}/demandas_normales{i+1}')
            ...
            filename = os.path.join(os.path.dirname(__file__), f'media_{tipo}/costo{j+1}_{i+1}')
